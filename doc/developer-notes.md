...
- [Development guidelines](#development-guidelines)
    - [General BullshitBucks Core](#general-bullshitbucks-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BullshitBucks Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play BSBs (tBSB)" on the test network, if you
...
BullshitBucks Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/bullshitbucks-project/bullshitbucks/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bullshitbucks
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bullshitbucks --log_level=test_suite
$ valgrind -v --leak-check=full src/bullshitbucksd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_bullshitbucks.coverage/index.html`.
```
...
- BitcoinMiner : Generates BSBs (if wallet is enabled).
...
However, in open source software such as BullshitBucks Core, where everyone uses
...
pay attention to for reviewers of BullshitBucks Core code.

General BullshitBucks Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: BullshitBucks Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against BullshitBucks Core so that they can be integrated
...
file descriptors it will cause problems with BullshitBucks's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:BullshitBucks-Project/BullshitBucks.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bullshitbucks-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bullshitbucks-cli` and the GUI debug console use this table to determine how to
