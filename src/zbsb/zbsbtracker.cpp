...
CzBSBTracker::CzBSBTracker(std::string strWalletFile)
{
...
CzBSBTracker::~CzBSBTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBSBTracker::Init()
{
...
bool CzBSBTracker::Archive(CMintMeta& meta)
{
...
bool CzBSBTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBSBTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBSBTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBSBTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBSBTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBSBTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBSBTracker::GetSerialHashes()
{
...
CAmount CzBSBTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBSBTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBSBTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBSBTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBSBTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBSBTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBSBTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBSBTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBSBTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBSBTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBSBTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBSBWallet* zBSBWallet)
{
    bool iszBSBWalletInitialized = (NULL != zBSBWallet);
    CMintMeta meta;
    ...
    if (! iszBSBWalletInitialized)
        zBSBWallet = new CzBSBWallet(strWalletFile);
    meta.isSeedCorrect = zBSBWallet->CheckSeed(dMint);
    if (! iszBSBWalletInitialized)
        delete zBSBWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBSBTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBSBTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBSBTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBSBTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBSBTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBSBTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBSBWallet* zBSBWallet = new CzBSBWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBSBWallet);
        }
        delete zBSBWallet;
        LogPrint("zero", "%s: added %d dzbsb from DB\n", __func__, listDeterministicDB.size());
    }
