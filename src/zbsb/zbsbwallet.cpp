...
CzBSBWallet::CzBSBWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbsb seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBSBSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBSBSeed_deprecated()) {
                    LogPrintf("%s: Updated zBSB seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbsb seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbsb wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBSBWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBSBCount(nCountLastUsed);
    else if (!walletdb.ReadZBSBCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBSBWallet::Lock()
{
    seedMaster = 0;
}

void CzBSBWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBSBWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBSB(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBSBWallet::LoadMintPoolFromDB()
{
...
void CzBSBWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBSBWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBSBWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbsbTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBSBWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBSB(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbsbTracker which also adds to database
    pwalletMain->zbsbTracker->Add(dMint, true);
...
void CzBSBWallet::SeedToZBSB(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBSBWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBSBWallet::GenerateDeterministicZBSB(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBSBWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBSBWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBSBWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
