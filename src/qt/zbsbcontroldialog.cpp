...
#include "ui_zbsbcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbsbTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbsbTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBSB.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBSB seed used to mint this zBSB is not the same as currently hold in the wallet";
            else
