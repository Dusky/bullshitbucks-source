...
// The zBSB block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBSB is a hash of the serial
...
{
    //Create an output returning the zBSB that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBSBOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBSB output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBSB", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBSBOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBSB output", __func__);
...
{
    CzBSBTracker* zbsbTracker = pwallet->zbsbTracker.get();
    CMintMeta meta;
    if (!zbsbTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbsbTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BSB stake is the outpoint
