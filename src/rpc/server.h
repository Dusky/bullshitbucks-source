...
/**
 * BullshitBucks RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZbsbSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzbsbseed(const UniValue& params, bool fHelp);
extern UniValue getzbsbseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzbsb(const UniValue& params, bool fHelp);
extern UniValue dzbsbstate(const UniValue& params, bool fHelp);
