...
public:
    ...
    bool WriteZBSBSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBSBSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBSBSeed_deprecated(uint256& seed);
    bool EraseZBSBSeed();
    bool EraseZBSBSeed_deprecated();

    bool WriteZBSBCount(const uint32_t& nCount);
    bool ReadZBSBCount(uint32_t& nCount);
